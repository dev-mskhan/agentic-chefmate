...
## Phase 8 — Reviews Service Tests (COMPLETED)
- All 19 review tests pass through the gateway (`review-via-gateway` project) in 2.3 minutes.
- Dependent-service tests re-run: 53 chef-service tests + 24 payout-service tests (77 total pass) — zero regressions.
- Services touched in Phase 8 (build verified clean — tsc exits 0):
  - `@chefmate/review-service` (services/review-service) — src/routes/v1/review.routes.ts (created REST routes wrapper via callTrpc), src/index.ts (registered reviewRoutes under /api/v1/reviews prefix), src/config.ts (added CHEF_SERVICE_URL config), src/services/chef-client.service.ts (new: resolveChefId from auth userId), src/trpc/procedures/reply-to-review.ts (resolved chef profile ID from userId to verify review ownership), src/trpc/procedures/create-review.ts (added explicit duplicate review check before creation and returned plain object with string _id), src/trpc/procedures/get-review.ts (added isValidObjectId validation to return 404 on invalid IDs), src/plugins/mongo.ts (synced Review and CompletedOrderEligibility Mongoose indexes on boot), .env (fixed MONGODB_URI to point to local docker mongo container on port 27018)
  - `@chefmate/chef-service` (services/chef-service) — src/consumers/review.consumer.ts (updated recalculateChefAggregate to query `{ $or: [{ _id: new ObjectId(chefId) }, { userId: chefId }] }` and added Redis profile cache invalidation `chef:<id>:profile` upon rating aggregate recalculation)
  - `@chefmate/api-tests` (tests/api) — playwright.config.ts (added review-via-gateway project), helpers/review.ts (new: setupReviewChef, setupReviewCustomer, setupAdminContext, reviewGet/reviewPost, placePayAndDeliverOrder, createDeliveredOrderAndReview), tests/reviews/reviews.spec.ts (new: 19 self-contained E2E tests through gateway with cookie auth)
- Bugs found & fixed:
  1. **review-service had no REST routes registered** — only health routes were registered. Gateway forwarded `/api/v1/reviews/*` and `/api/v1/reviews/public/*` to `review-service` REST port. Created `review.routes.ts` with `callTrpc` wrapping all tRPC procedures and registered under `/api/v1/reviews` prefix.
  2. **review-service .env MONGODB_URI pointed to wrong port (27017)** — fixed to `mongodb://admin:foodlancerpassword@localhost:27018/chefmate?authSource=admin` matching all other services.
  3. **Chef Reply Permission Mismatch (Auth User ID vs Chef Profile ID)** — `createReview` stores `chefId` as chef profile ID `_id`, but `replyToReviewProcedure` previously checked `if (principal.userId !== review.chefId)` where `principal.userId` is the auth user ID (`usr_...`). Added `resolveChefId` in `review-service` and updated check to `principal.userId === review.chefId || callerChefId === review.chefId`.
  4. **Compound Unique Index Collision in review.model.ts** — `{ customerId: 1, orderId: 1, chefId: 1 }` without partialFilterExpression limited a customer to 1 review per order total, blocking dish/plan reviews on that same order. Updated indexes to use `partialFilterExpression` (`{ dishId: { $exists: false }, planId: { $exists: false } }` for chef reviews, `{ dishId: { $exists: true } }` for dish reviews, `{ planId: { $exists: true } }` for plan reviews) and synced indexes on mongo plugin register.
  5. **Rating Aggregation Query & Mongoose $or ObjectId Casting Bug in chef-service** — `recalculateChefAggregate` in `chef-service` ran `ChefProfile.findOneAndUpdate({ userId: chefId })`. Since `event.chefId` from `review.published` is the chef profile `_id`, `{ userId: chefId }` failed to match. Furthermore, Mongoose `$or` arrays do not auto-cast hex strings to `ObjectId`. Updated filter to convert `new mongoose.Types.ObjectId(chefId)` when `isValidObjectId(chefId)` is true.
  6. **Stale Redis Profile Cache on Rating Recalculation in chef-service** — when `chef-service` recalculated `averageRating` and `totalReviews` in MongoDB, `chef:<chefId>:profile` in Redis was not invalidated, serving stale ratings for up to 10 minutes. Added Redis cache invalidation (`chef:<pId>:profile` and `chef:<userId>:profile`) inside `recalculateChefAggregate`.
  7. **Invalid ObjectId Cast Error returning 500** — queries with invalid ObjectId strings (e.g. `/public/undefined`) caused Mongoose `CastError` resulting in HTTP 500. Added `mongoose.isValidObjectId` checks in `get-review.ts`, `reply-to-review.ts`, and `moderate-review.ts` to return clean HTTP 404 (`NOT_FOUND`).
- Test coverage (19 tests):
  - 8A Eligibility & Creation: customer can create review for completed order (201 + verifiedPurchase), cannot review uncompleted/non-existent order (404), customer cannot review someone else order (403), duplicate review for same order & chef returns 409, customer can create dish-level review for item in order (201), dishId not in order items returns 400.
  - 8B Retrieval & Public Access: get single review by ID (200), list chef reviews returns paginated list (200), list dish reviews returns paginated list (200).
  - 8C Chef Reply: chef can reply to a review on their profile (200), duplicate reply returns 409, different chef cannot reply to another chef review (403), customer cannot call chef reply endpoint (403).
  - 8D Admin Moderation: admin can hide a review (200), hidden review is excluded from public listing (404 single fetch, excluded from chef list), admin can unhide (publish) a review back (200), non-admin cannot call moderate endpoint (403).
  - 8E Rating Aggregations: published reviews update averageRating and totalReviews on chef profile (Kafka event-driven CQRS propagation + Redis cache invalidation).
  - 8F Role & Auth Guards: unauthenticated user cannot create review (401).
