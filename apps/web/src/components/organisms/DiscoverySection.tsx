    ...
    <section id="discover" className="relative z-10 mx-auto -mt-2 max-w-[1500px] scroll-mt-24 px-4 pb-20 sm:px-8 lg:px-12 lg:pb-24 2xl:px-16">
      <div className="discovery-panel rounded-[2rem] bg-espresso p-5 text-cream shadow-lg sm:p-8 lg:p-12">
        <div className="flex flex-wrap items-end justify-between gap-6">
          <div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-saffron">Find food near you</p><h2 className="mt-3 max-w-2xl font-display text-4xl leading-tight tracking-[-0.03em] sm:text-5xl">Choose food for your week.</h2></div>
        ...
        <div className="mt-7 grid gap-3 sm:grid-cols-3" aria-live="polite">
          {results.map((item) => {
            const href = mode === 'chefs' ? item.name === 'Ayesha Khan' ? '/chefs/chef-ayesha-khan' : '/discover?type=chefs' : mode === 'dishes' ? item.name === 'Smoky karahi' ? '/dishes/dish-smoky-karahi' : '/discover?type=dishes' : item.name === 'The Sunday Dastarkhwan' ? '/plans/plan-sunday-dastarkhwan' : '/discover?type=meal-plans'
            return <Link to={href} key={item.name} className="dish-card overflow-hidden rounded-2xl bg-cream text-charcoal transition-transform hover:-translate-y-1"><div className="aspect-[16/10] overflow-hidden"><img className="h-full w-full object-cover" src={item.image} alt={item.name} loading="lazy" /></div><div className="p-4"><div className="flex items-start justify-between gap-3"><h3 className="font-semibold">{item.name}</h3><span className="shrink-0 text-sm font-semibold text-terracotta">{item.price}</span></div><p className="mt-1 text-sm text-charcoal-70">{item.meta}</p></div></Link>
          })}
