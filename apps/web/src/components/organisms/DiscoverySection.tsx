...
const discoveryData: Record<SearchMode, DiscoveryItem[]> = {
  chefs: [
    ...
    { name: 'Browse all chefs', meta: 'Lahore and Karachi', price: '→', image: 'https://images.unsplash.com/photo-1556761223-4c4282c73f77?auto=format&fit=crop&w=900&q=80' },
  ],
  dishes: [
...
export function DiscoverySection() {
  ...
  // Magnetic hover card animations per DESIGN.md §3.3
  useEffect(() => {
    ...
    cards.forEach((card, i) => {
      const enter = () => {
        gsap.to(card, {
          y: -8,
          scale: 1.02,
          boxShadow: '0 20px 40px rgba(33,28,23,0.15)',
          duration: 0.4,
          ease: 'power2.out',
        })
      }
      const leave = () => {
        gsap.to(card, {
          y: 0,
          scale: 1,
          boxShadow: '0 1px 2px rgba(33,28,23,0.06)',
          duration: 0.4,
          ease: 'power2.out',
        })
      }
  ...
  return (
    <section id="discover" className="relative z-10 mx-auto -mt-2 max-w-[1500px] scroll-mt-24 px-4 pb-20 sm:px-8 lg:px-12 lg:pb-24 2xl:px-16">
      <SectionReveal start="top 88%">
        <div className="discovery-panel rounded-[2rem] bg-espresso p-5 text-cream shadow-xl sm:p-8 lg:p-12">
          <div className="flex flex-wrap items-end justify-between gap-6">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-saffron">Find food near you</p>
              <h2 className="mt-3 max-w-2xl font-display text-4xl leading-tight tracking-[-0.03em] sm:text-5xl">Choose food for your week.</h2>
            </div>
            <p className="max-w-xs text-sm leading-6 text-cream/65">Search chefs, dishes, or meal plans by name and city.</p>
          </div>

          <div className="mt-8 flex flex-wrap gap-2" role="tablist" aria-label="Discovery type">
            {([['chefs', 'Chefs by location'], ['dishes', 'Dishes'], ['meal-plans', 'Meal plans']] as const).map(([value, label]) => (
              <button
                key={value}
                type="button"
                role="tab"
                aria-selected={mode === value}
                onClick={() => { setMode(value); setSubmittedQuery('') }}
                className={`rounded-pill px-4 py-2 text-sm font-semibold transition-colors ${mode === value ? 'bg-saffron text-charcoal' : 'bg-cream/10 text-cream/70 hover:bg-cream/20 hover:text-cream'}`}
              >
                {label}
              </button>
            ))}
          ...
          <form className="mt-4 flex flex-col gap-3 sm:flex-row" onSubmit={handleSearch}>
            ...
            <div className="flex min-h-14 flex-1 items-center gap-3 rounded-2xl bg-cream px-4 text-charcoal">
              <svg aria-hidden="true" className="h-5 w-5 shrink-0 text-terracotta" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><circle cx="11" cy="11" r="6.5" /><path d="m16 16 4 4" /></svg>
              <input
                id="discovery-search"
                value={query}
                onChange={(event) => setQuery(event.target.value)}
                placeholder={mode === 'chefs' ? 'Try "Lahore" or "Karachi"' : mode === 'dishes' ? 'Try "karahi" or "daal"' : 'Try "weekly" or "family"'}
                className="w-full bg-transparent text-base outline-none placeholder:text-charcoal-70/60"
              />
            </div>
            <Button type="submit" className="min-h-14 px-7">Search</Button>
          </form>

          <div ref={gridRef} className="mt-7 grid gap-3 sm:grid-cols-3" aria-live="polite">
            {results.map((item) => {
              const href = mode === 'chefs'
                ? item.name === 'Ayesha Khan' ? '/chefs/chef-ayesha-khan' : '/discover?type=chefs'
                : mode === 'dishes'
                ? item.name === 'Smoky karahi' ? '/dishes/dish-smoky-karahi' : '/discover?type=dishes'
                : item.name === 'The Sunday Dastarkhwan' ? '/plans/plan-sunday-dastarkhwan' : '/discover?type=meal-plans'
              return (
                <Link
                  to={href}
                  key={item.name}
                  className="dish-card overflow-hidden rounded-2xl bg-cream text-charcoal will-change-transform"
                >
                  <div className="aspect-[16/10] overflow-hidden">
                    <img className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-105" src={item.image} alt={item.name} loading="lazy" />
                  </div>
                  <div className="p-4">
                    <div className="flex items-start justify-between gap-3">
                      <h3 className="font-semibold">{item.name}</h3>
                      <span className="shrink-0 text-sm font-semibold text-terracotta">{item.price}</span>
                    </div>
                    <p className="mt-1 text-sm text-charcoal-70">{item.meta}</p>
                  </div>
                </Link>
              )
            })}
            {results.length === 0 && <p className="rounded-2xl bg-cream/10 p-5 text-sm text-cream/75 sm:col-span-3">No results found. Try a different city or search term.</p>}
          </div>
        </div>
      </SectionReveal>
    </section>
