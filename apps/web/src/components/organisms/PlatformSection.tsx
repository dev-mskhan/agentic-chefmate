export function PlatformSection() {
  return (
    <section id="our-story" className="mx-auto grid max-w-[1500px] scroll-mt-24 gap-12 px-4 py-20 sm:px-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-center lg:gap-20 lg:px-12 lg:py-28 2xl:px-16">
      ...
      <div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-terracotta">Food with a point of view</p><h2 className="mt-4 max-w-xl font-display text-5xl leading-[0.98] tracking-[-0.035em] sm:text-6xl">Meet the human side of dinner.</h2><p className="mt-6 max-w-lg text-lg leading-8 text-charcoal-70">ChefMate brings local cooks, thoughtful menus, and flexible plans into one place.</p><div className="mt-8 flex flex-wrap gap-2 text-sm font-semibold"><span className="rounded-pill bg-terracotta-10 px-4 py-2 text-terracotta-dark">Local chefs</span><span className="rounded-pill bg-cream-dim px-4 py-2 text-charcoal-70">Flexible plans</span><span className="rounded-pill bg-cream-dim px-4 py-2 text-charcoal-70">Real connection</span></div></div>
    </section>
  )
}
